...
class TeraMessage {
  constructor(mod) {
    ...
    this.guildYEL = function (msg) {
      mod.send("C_CHAT", 1, {
        channel: 2,
        message: '<FONT color="#ffcc00"><ChatLinkAction param="1#####0@0@name">' + msg + "</ChatLinkAction>",
      })
    }
    this.guildRED = function (msg) {
      mod.send("C_CHAT", 1, {
        channel: 2,
        message: '<FONT color="#ee1c24"><ChatLinkAction param="1#####0@0@name">' + msg + "</ChatLinkAction>",
      })
    }
